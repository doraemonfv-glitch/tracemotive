...
from agentlens.canonical import LLMDetails, LLMUsage
...
class ExplosiveSource:
    def __str__(self):
        raise AssertionError("AgentLens inspected disabled content")

    def __repr__(self):
        raise AssertionError("AgentLens represented disabled content")

    def __iter__(self):
        raise AssertionError("AgentLens iterated disabled content")
...
class SDKTests(unittest.TestCase):
    ...
    def test_disabled_by_default_is_a_noop_and_preserves_user_exceptions(self):
        body_ran = []
        with agentlens.trace("disabled"):
            body_ran.append(True)
            with agentlens.span("ignored") as span_handle:
        ...
        with self.assertRaisesRegex(RuntimeError, "user failure"):
            with agentlens.trace("disabled-error"):
    ...
    def test_configure_validates_loopback_and_freezes_after_first_event(self):
        with self.assertRaises(agentlens.AgentLensConfigurationError):
            agentlens.configure(enabled=True, endpoint="https://127.0.0.1:8765")
        with self.assertRaises(agentlens.AgentLensConfigurationError):
            agentlens.configure(enabled=True, endpoint="http://192.0.2.1:8765")
        with self.assertRaises(agentlens.AgentLensConfigurationError):
            agentlens.configure(enabled=True, endpoint="http://localhost.evil:8765")

        agentlens.configure(enabled=True, endpoint="http://LOCALHOST:8765")
        with agentlens.trace("freeze"):
            pass
        agentlens.configure(enabled=True, endpoint="http://LOCALHOST:8765")
        with self.assertRaises(agentlens.AgentLensConfigurationError):
            agentlens.configure(enabled=True, capture_content=True, endpoint="http://LOCALHOST:8765")
    ...
    def test_first_event_and_configure_race_has_one_atomic_winner(self):
        ...
        self.assertIsInstance(configure_result[0], agentlens.AgentLensConfigurationError)
    ...
    def test_concurrent_identical_configure_calls_are_safe(self):
        ...
        def configure_identically():
            barrier.wait()
            agentlens.configure(enabled=True, endpoint="http://127.0.0.1:8765")

        with ThreadPoolExecutor(max_workers=2) as pool:
            results = list(pool.map(lambda _: configure_identically(), (1, 2)))
        self.assertEqual(results, [None, None])
        with agentlens.trace("after-identical-configure"):
            pass

    def test_concurrent_same_and_conflicting_configure_calls_after_freeze(self):
        agentlens.configure(enabled=True)
        with agentlens.trace("freeze-before-race"):
            pass
        barrier = threading.Barrier(2)
        ...
        self.assertIsInstance(conflict, agentlens.AgentLensConfigurationError)

    def test_trace_normal_and_exceptional_exit_have_explicit_status(self):
        agentlens.configure(enabled=True)
        with agentlens.trace("success", metadata={"request": "safe"}) as current:
            self.assertIsNotNone(current)
            self.assertIs(sdk.current_trace(), current)
        with agentlens.trace("failure"):
            pass
        with self.assertRaisesRegex(ValueError, "original"):
            with agentlens.trace("raised"):
    ...
    def test_span_lifecycle_parenting_and_child_error_do_not_change_trace(self):
        agentlens.configure(enabled=True)
        with agentlens.trace("workflow") as current_trace:
            with agentlens.span("outer") as outer:
                self.assertIs(sdk.current_span(), outer)
                with agentlens.span("inner") as inner:
                    self.assertEqual(inner.parent_span_id, outer.span_id)
                with self.assertRaisesRegex(RuntimeError, "child"):
                    with agentlens.span("failed-child"):
    ...
    def test_span_error_preserves_type_when_exception_stringification_fails(self):
        agentlens.configure(enabled=True)
        observed = None
        with agentlens.trace("bad-error"):
            try:
                with agentlens.span("bad-child"):
                    raise BadError()
    ...
    def test_metaclass_name_lookup_failure_preserves_error_type_and_message(self):
        ...
        agentlens.configure(enabled=True)
        observed = None
        with agentlens.trace("metaclass-name"):
            try:
                with agentlens.span("child"):
    ...
    def test_metaclass_and_stringification_failures_preserve_type_with_sink_failure(self):
        ...
        agentlens.configure(enabled=True)
        observed = None
        with agentlens.trace("metaclass-and-string"):
            try:
                with agentlens.span("child"):
    ...
    def test_span_error_preserves_type_when_message_sanitization_fails(self):
        ...
        with patch.object(sdk.privacy, "sanitize_text", side_effect=fail_only_for_message):
            with agentlens.trace("sanitize-error"):
                try:
                    with agentlens.span("child"):
    ...
    def test_flush_isolates_lookup_and_invocation_failures(self):
        ...
        self.assertFalse(agentlens.flush())
    ...
    def test_nested_trace_is_independent_and_restores_outer_span(self):
        agentlens.configure(enabled=True)
        with agentlens.trace("outer") as outer_trace:
            with agentlens.span("A") as outer_span:
                with agentlens.trace("inner") as inner_trace:
                    self.assertNotEqual(inner_trace.trace_id, outer_trace.trace_id)
                    self.assertIs(sdk.current_trace(), inner_trace)
                    self.assertIsNone(sdk.current_span())
                    with agentlens.span("inner-root") as inner_span:
    ...
    def test_exceptional_nested_trace_restores_exact_outer_context(self):
        agentlens.configure(enabled=True)
        with agentlens.trace("outer") as outer_trace:
            with agentlens.span("outer-span") as outer_span:
                inner_trace_id = None
                try:
                    with agentlens.trace("inner") as inner_trace:
                ...
                self.assertIs(sdk.current_trace(), outer_trace)
                self.assertIs(sdk.current_span(), outer_span)
                with agentlens.span("outer-child") as outer_child:
    ...
    def test_capture_is_disabled_by_default_and_redacted_before_sink(self):
        ...
        with agentlens.trace("disabled-capture"):
            with agentlens.span("span", input={"password": "secret-input"}) as handle:
    ...
    def test_capture_enabled_supports_null_and_exact_lifecycle_output_states(self):
        agentlens.configure(enabled=True, capture_content=True)
        with agentlens.trace("capture"):
            with agentlens.span("captured", input=None) as handle:
        ...
        sdk._set_event_sink(self.sink)
        agentlens.configure(enabled=True, capture_content=True)
        with agentlens.trace("redaction"):
            with agentlens.span(
                "redacted",
    ...
    def test_manual_capture_reports_source_serialization_and_size_reasons(self):
        agentlens.configure(enabled=True, capture_content=True)
        with agentlens.trace("capture-reasons"):
            with agentlens.span("unavailable"):
                pass
            with agentlens.span("serialization-error", input=object()):
                pass
            with agentlens.span("size-limit", input="x" * (MAX_CONTENT_BYTES - 1)):
                pass
        started = {
    ...
    def test_disabled_paths_do_not_inspect_content_sources(self):
        with agentlens.trace(ExplosiveSource(), ExplosiveSource()):
            with agentlens.span("disabled-span", input=ExplosiveSource()) as handle:
                handle.set_output(ExplosiveSource())
        self.assertEqual(self.sink.events, [])

        agentlens.configure(enabled=True, capture_content=False)
        with agentlens.trace("capture-disabled"):
            with agentlens.span("disabled-content", input=ExplosiveSource()) as handle:
                handle.set_output(ExplosiveSource())
    ...
    def test_enabled_standalone_span_remains_the_accepted_noop(self):
        agentlens.configure(enabled=True)
        body_ran = []
        with agentlens.span("standalone") as handle:
            body_ran.append(True)
    ...
    def test_ids_and_timestamps_are_canonical(self):
        ...
        for index in range(20):
            with agentlens.trace(f"identity-{index}"):
                with agentlens.span(f"identity-{index}"):
                    pass
        event_ids = set()
        trace_ids = set()
        span_ids = set()
    ...
    def test_context_isolation_for_async_tasks_and_threads(self):
        ...
        async def task_body(label, barrier):
            with agentlens.trace(label):
                trace_id = sdk.current_trace().trace_id
                await barrier.wait()
                with agentlens.span(label) as handle:
    ...
    def test_explicit_async_task_inherits_trace_and_span_context_then_restores_parent(self):
        ...
        async def child_created_in_trace(parent_trace, result):
            ...
            with agentlens.trace("task-inner") as inner_trace:
                result.append((sdk.current_trace(), sdk.current_span()))
                with agentlens.span("task-root") as task_root:
                    result.append(task_root.parent_span_id)
            result.append((sdk.current_trace(), sdk.current_span()))

        async def child_created_in_span(parent_span, result):
            result.append((sdk.current_trace(), sdk.current_span()))
            with agentlens.span("task-child") as task_child:
        ...
        async def run():
            trace_child_result = []
            span_child_result = []
            with agentlens.trace("parent") as parent_trace:
                ...
                self.assertIsNone(sdk.current_span())
                with agentlens.span("parent-span") as parent_span:
    ...
    def test_overlapping_threads_restore_only_their_own_context(self):
        ...
        def worker(label):
            with agentlens.trace(label) as current_trace:
                with agentlens.span(label) as current_span:
                    observations[label] = (
    ...
    def test_internal_failures_are_isolated_and_user_exceptions_survive(self):
        ...
        with patch.object(sdk, "_new_trace_id", side_effect=RuntimeError("id failure")):
            with agentlens.trace("isolated"):
                pass
        self.assertEqual(self.sink.events, [])

        with agentlens.trace("sink failure"):
            pass
        sdk._set_event_sink(FailingSink())
        self.assertFalse(agentlens.flush())
        with self.assertRaisesRegex(RuntimeError, "user"):
            with agentlens.trace("preserve"):
    ...
    def test_flush_isolated_without_issue_07_transport(self):
        ...
        sdk._set_event_sink(Sink())
        self.assertTrue(agentlens.flush(0.25))
